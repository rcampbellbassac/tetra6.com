...
/**
 * Health check. A plain GET returns service status without touching the
 * Sheet's contents or sending mail, so it's safe to poll on a schedule.
 *
 * This exists because the failure that actually bit us was silent: the
 * deployment's "Who has access" reset to "Only myself", every submission
 * started returning 403, and nothing surfaced it. Polling this catches that
 * class of breakage in minutes rather than whenever someone notices the
 * inquiries stopped arriving.
 */
function doGet(e) {
  var status = { ok: true, service: "tetra6-contact-form" };

  // "The script runs" and "the script can still write" are different things,
  // and only the second one means submissions are actually safe — so touch
  // the Sheet (read-only) rather than just returning a static OK.
  try {
    status.rows = SpreadsheetApp.getActiveSpreadsheet()
      .getActiveSheet()
      .getLastRow();
  } catch (err) {
    status.ok = false;
    status.error = "sheet unreachable";
  }
